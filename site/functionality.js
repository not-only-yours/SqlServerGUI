...
function reservePlace(person,mother,father,group) {
    ...
    res.createHumanInDatabase(person + ", " + mother + ", " + father + ";")
    res.chainCreateClient()
    res.updateReserveOfGroup(group)
    res.chainCreateRequest()
}

class Reservation {
    chainCreateRequest() {
        this.getIdOfNewChild()
            .then(data => {
                console.log(data)
                this.createRequest(data,group)
            })
    }

    chainCreateClient() {
        this.getIdOfNewHuman()
            .then(data =>
                this.createClient(data)
            )
    }
    createHumanInDatabase(str) {
